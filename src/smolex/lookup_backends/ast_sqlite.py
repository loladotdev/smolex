...
def insert_into_database(db_connection, filename: str, entity_type: str, name: str, ast_pickle: bytes):
    db_connection.execute('INSERT INTO Entities (FileName, Type, Name, AST) VALUES (?, ?, ?, ?)',
        ...
        for root, dirs, files in os.walk(directory):
            for file in files:
                if any(file.endswith(extension) for extension in config.extensions):
                    ...
                    for node in [n for n in ast.walk(root_node) if isinstance(n, (ast.ClassDef, ast.FunctionDef))]:
                        insert_into_database(db_connection=db_connection, filename=filename, name=node.name,
                                             entity_type=type(node).__name__, ast_pickle=pickle.dumps(node))
...
def generate_class_interface(ast_node: ast.AST) -> str:
    ...
    for node in ast_node.body:
        if isinstance(node, ast.FunctionDef):
            ...
            output.append(f'    def {func_name}(self, {", ".join(args)}) -> {ret_type}:')
            output.append('        <...>')
